...
- (void)browserExit
{
    if (self.callbackId != nil) {
        NSMutableDictionary* dResult = [NSMutableDictionary new];
        [dResult setValue:@"exit" forKey:@"type"];
        if (self->_exitMode != nil) {
            [dResult setValue:self->_exitMode forKey:@"mode"];
            self->_exitMode = nil;
        }
        CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:dResult];
...
- (void)setExitMode:(NSString*)mode
{
    self->_exitMode = mode;
}
...
- (void)createViews
{
   ...
   if (@available(iOS 11.0, *)) {
    ...
    TitleBarView* toolbar = [[TitleBarView alloc] initWithFrame:toolbarFrame];
...
- (void)webView:(WKWebView *)theWebView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if (isTopLevelNavigation) {
        self.currentURL = url;
        self.txtUrl.text = [url absoluteString];
    }

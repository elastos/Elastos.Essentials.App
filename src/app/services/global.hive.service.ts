...
export class GlobalHiveService extends GlobalService {
  ...
  public getSyncDataToHiveWasPrompted(): Promise<boolean> {
    return this.storage.getSetting(DIDSessionsStore.signedInDIDString, "hiveservice", "syncdataprompted", false);
  }

  public setSyncDataToHiveWasPrompted() {
    return this.storage.setSetting(DIDSessionsStore.signedInDIDString, "hiveservice", "syncdataprompted", true);
  }
  ...
  public async showHiveSyncInfoPopup(): Promise<boolean> {
    ...
    if (confirmed) {
      await this.prefs.setUseHiveSync(DIDSessionsStore.signedInDIDString, true);
    }

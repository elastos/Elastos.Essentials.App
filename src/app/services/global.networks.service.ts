...
export class GlobalNetworksService {
  ...
  public async init(): Promise<void> {
    this.activeNetworkTemplate.next(await this.prefs.getPreference(null, null, 'network.template', true));
  ...
  public async setActiveNetworkTemplate(networkTemplate: string): Promise<void> {
    if (DIDSessionsStore.signedInDIDString !== null) {
      throw new Error(
        'setActiveNetworkTemplate() is a global preference shared by all users and may be called only from DID sessions, when no user is signed in'
      );
    }

...
export class GlobalThemeService extends GlobalService {
  ...
  public async fetchThemeFromPreferences() {
    ...
    let themeKey = await this.prefs.getPreference<string>(DIDSessionsStore.signedInDIDString, "ui.theme");
    let themeConfig = availableThemes.find(theme => theme.key === themeKey);

    let themeVariant = await this.prefs.getPreference<"light" | "dark">(DIDSessionsStore.signedInDIDString, "ui.variant");
  ...
  public async setThemeConfig(theme: ThemeConfig) {
    ...
    // Persist
    await this.prefs.setPreference(DIDSessionsStore.signedInDIDString, "ui.theme", theme.key);
  ...
  private async setThemeVariant(themeVariant: "light" | "dark") {
    ...
    // Persist
    await this.prefs.setPreference(DIDSessionsStore.signedInDIDString, "ui.variant", themeVariant);

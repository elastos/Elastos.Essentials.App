...
export class CredentialAccessRequestPage {
  ...
  public requestDapp: RequestDapp = null;
  ...
  ionViewWillEnter() {
    ...
    this.zone.run(async () => {
      ...
      console.log("Request Dapp", this.requestDapp);
  ...
  getRequestedTheme(): Promise<void> {
    ...
    return new Promise((resolve) => {
      const customization = this.requestDapp.customization
      if(customization) {
        if(
  ...
  async organizeRequestedClaims() {
    ...
    for (let key of Object.keys(this.requestDapp.claims)) {
      const claim = this.requestDapp.claims[key];
  ...
  async acceptRequest() {
    ...
    setTimeout(() => {
      ...
      AuthService.instance.checkPasswordThenExecute(async ()=>{
        ...
        presentation = await this.didService.getActiveDid().createVerifiablePresentationFromCredentials(selectedCredentials, this.authService.getCurrentUserPassword(), this.requestDapp.nonce, this.requestDapp.realm);
        ...
        if (this.requestDapp.originalJwtRequest) {
          console.log('Intent is called by external intent', this.requestDapp.originalJwtRequest);
          payload["req"] = "elastos://credaccess/"+this.requestDapp.originalJwtRequest;

          let parsedJwt = await didManager.parseJWT(false, this.requestDapp.originalJwtRequest);
          if (parsedJwt) {
        ...
        const jwtToken = await this.didService.getActiveDid().getDIDDocument().createJWT(
          payload,
          this.requestDapp.jwtExpirationDays,
...
/*     this.popup.onWillDismiss().then(async () => {
  ...
  getDappName() {
    if(this.requestDappName) {
      ...
      return this.requestDapp.appPackageId;
    }
  }

...
export class AppIdCredentialIssueRequestPage {
    ...
    async checkIntentSenderAppDid() {
        if (!await this.uxService.isIntentResponseGoingOutsideElastos(this.requestDapp.params)) {
            // Get real app did from runtime
            let appDid = await this.uxService.getAppDid(this.requestDapp.appPackageId);
            if (!appDid) {
    ...
    organizeDisplayableInformation() {
        ...
        this.displayableCredential = {
            appinstancedid: this.requestDapp.appinstancedid,
            appdid: this.requestDapp.appdid,
            expirationDate: new Date(now + oneMonthAsMs),
        };
    }

    async acceptRequest() {
        this.appIDService.generateAndSendApplicationIDCredentialIntentResponse(this.requestDapp.params);
    }

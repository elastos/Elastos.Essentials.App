...
import { FindOptions, InsertOptions, UpdateOptions, VaultServices } from '@elastosfoundation/elastos-hive-js-sdk';
...
export class HiveDataSync {
     ...
     */
    constructor(private userVault: VaultServices, private storage: GlobalStorageService, private showDebugLogs: boolean = false) {
        if (!userVault) {
    ...
    private saveSettingsEntry(key: string, value: any): Promise<void> {
        return this.storage.setSetting(this.userVault.getUserDid(), "hivedatasync", key, value);
    }

    // Convenient promise-based way to get a setting from the app manager
    private loadSettingsEntry(key: string): Promise<any> {
        return this.storage.getSetting(this.userVault.getUserDid(), "hivedatasync", key, null);
    }

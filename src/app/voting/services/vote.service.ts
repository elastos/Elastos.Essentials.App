...
import { ElastosStandardNetworkWallet } from 'src/app/wallet/model/wallets/elastos/standard/networkwallets/elastos.networkwallet';
...
export class VoteService {
    ...
    public async navigateTo(networkWallet: ElastosStandardNetworkWallet) {
        ...
        // For now, voting allowed only with standard wallets
        if (networkWallet.masterWallet.type !== WalletType.STANDARD) {
            await this.globalPopupService.ionicAlert('wallet.text-warning', 'Voting is currently allowed only with standard wallets');
            return;
        }

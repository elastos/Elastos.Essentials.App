...
export class StakeService {
  ...
  public async getBalanceByAddress(address: string, spendable = false): Promise<number> {
    ...
    try {
      const balanceList = await this.voteService.sourceSubwallet.callGetBalanceByAddress(
      ...
      if (this.votesRight.totalVotesRight > 0) {
        if (result[0].remainvoteright) {
          ...
          if (arr.length > 0) {
            ...
            this.votesRight.maxStakedRatio = this.uxService.getPercentage(
              this.votesRight.maxStaked,
              this.votesRight.totalVotesRight
            );
        ...
        if (result[0].usedvotesinfo) {
          ...
          this.votesRight.voteInfos.push({
            index: 1,
            title: 'staking.cr-council',
            list: result[0].usedvotesinfo.usedcrvotes
          });
          this.votesRight.voteInfos.push({
            index: 2,
            title: 'staking.cr-proposal',
            list: result[0].usedvotesinfo.usedcrcproposalvotes
          });
          this.votesRight.voteInfos.push({
            index: 3,
            title: 'staking.cr-impeachment',
            list: result[0].usedvotesinfo.usdedcrimpeachmentvotes
          });

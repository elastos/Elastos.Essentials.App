...
export class ProposalService {
  ...
  public async fetchProposals(status: ProposalStatus, page: number, results = 10): Promise<ProposalSearchResult[]> {
    try {
      ...
      var url =
        this.getCrRpcApi() + '/api/v2/proposal/all_search?status=' + status + '&page=' + page + '&results=' + results;
  ...
  public async fetchSearchedProposal(
    page = 1,
    ...
    try {
      var url =
        this.getCrRpcApi() +
        '/api/v2/proposal/all_search?page=' +
        page +
        '&results=10&status=' +
        status +
        '&results=10&search=' +
      ...
      this.http
        ...
        .subscribe(
          ...
          err => {
            ...
            if (err.error && err.error.message) reject(err.error.message); // Improved message
            else reject(err); // Raw error
          }
        );
    });
  }
  ...
  public navigateToProposalDetailPage(proposal: ProposalSearchResult) {
    void this.nav.navigateTo(App.CRPROPOSAL_VOTING, '/crproposalvoting/proposal-details', {
  ...
  getRemainingTimeString(remainingBlockCount: number): Promise<string> {
    ...
    if (remainingBlockCount >= 720 * 2) {
      ...
      ret =
        '1 ' +
        this.translate.instant('crproposalvoting.day') +
        ' ' +
        Math.floor((remainingBlockCount % 720) / 30) +
        ' ' +
        this.translate.instant('crproposalvoting.hours');
    } else if (remainingBlockCount == 720) {
  ...
  async fetchWithdraws(proposalHash: string): Promise<number> {
    ...
    try {
      ...
      if (
        ...
        result.proposalstate.proposal &&
        !Util.isEmptyObject(result.proposalstate.proposal.budgets)
      ) {

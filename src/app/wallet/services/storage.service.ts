...
export class LocalStorage {
  ...
  public async get(key: string): Promise<any> {
    ...
    let val = await this.storage.getSetting(
      DIDSessionsStore.signedInDIDString,
      NetworkTemplateStore.networkTemplate,
      "wallet",
      key,
    ...
    if (val === null) return null; // Key not found in setting
    else {
  ...
  public async getWalletsList(networkTemplate: string): Promise<string[]> {
    let key = "wallets-list-" + networkTemplate;
    let rawWallets = await this.storage.getSetting(
      DIDSessionsStore.signedInDIDString,
      NetworkTemplateStore.networkTemplate,
      "wallet",
      key,
  ...
  public saveMasterWallet(
    masterWalletId: string,
    masterWalletInfo: SerializedMasterWallet
  ): Promise<void> {
    let key = "master-wallet-info-" + masterWalletId;
    return this.set(key, JSON.stringify(masterWalletInfo));
  }

  public async loadMasterWallet(
    masterWalletId: string
  ): Promise<SerializedMasterWallet> {
    let key = "master-wallet-info-" + masterWalletId;
    return await this.get(key);
  }
  ...
  public deleteMasterWallet(masterWalletId: string): Promise<void> {
    let key = "master-wallet-info-" + masterWalletId;
    return this.storage.deleteSetting(
      DIDSessionsStore.signedInDIDString,
      NetworkTemplateStore.networkTemplate,
      "wallet",
      key
   ...
   */
  public setExtendedNetworkWalletInfo(
    masterId: string,
    networkTemplate: string,
    networkName: string,
  ...
  public async getExtendedNetworWalletInfo(
    masterId: string,
    networkTemplate: string,
    networkName: string
  ): Promise<ExtendedNetworkWalletInfo> {
    let key = "extended-network-wallet-info-" + masterId + "-" + networkName;
    return await this.get(key);
  }
   ...
   */
  public saveAASafe(masterWalletId: string, aaSafeData: any): Promise<void> {
    let key = "aa-safe-" + masterWalletId;
    return this.set(key, JSON.stringify(aaSafeData));
  }

  /**
   * Load AA safe data from persistent storage
   */
  public async loadAASafe(masterWalletId: string): Promise<any> {
    let key = "aa-safe-" + masterWalletId;
    return await this.get(key);
  }

  /**
   * Delete AA safe data when wallet is removed
   */
  public deleteAASafe(masterWalletId: string): Promise<void> {
    let key = "aa-safe-" + masterWalletId;
    return this.storage.deleteSetting(
      DIDSessionsStore.signedInDIDString,
      NetworkTemplateStore.networkTemplate,

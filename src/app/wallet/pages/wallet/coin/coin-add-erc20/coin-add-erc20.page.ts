...
export class CoinAddERC20Page implements OnInit {
    ...
    async scanCoinAddress() {
        let res: { result: { scannedContent: string } } = await this.globalIntentService.sendIntent('https://scanner.web3essentials.io/scanqrcode', {});
    ...
    async addCoin() {
        if (this.coinAlreadyAdded(this.coinAddress)) {
            ...
            const newCoin = new ERC20Coin(this.network, this.coinSymbol, this.coinName, this.coinAddress, this.coinDecimals, true);

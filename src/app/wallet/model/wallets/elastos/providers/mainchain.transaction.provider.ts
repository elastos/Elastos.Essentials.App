...
export class MainchainProvider extends MainAndDIDChainProvider<MainchainSubWallet> {
  ...
  public canFetchMoreTransactions(subWallet: AnySubWallet): boolean {
    return (this.needtoLoadMoreAddresses.length > 0);
  }
  ...
  private mergeTransactionList(txList: PaginatedTransactions<ElastosTransaction>[]) {
    if (!txList)
    ...
    /* if (!this.paginatedTransactions.has(this.subWallet.id))
    this.paginatedTransactions.set(this.subWallet.getTransactionsCacheKey(), {
      total: 0,
      transactions: []
    }); */

...
export class EthereumGoerliNetwork extends EthereumBaseNetwork {
  ...
  public getAPIUrlOfType(type: NetworkAPIURLType): string {
    if (type === NetworkAPIURLType.RPC) return EthereumAPI.getApiUrl(EthereumAPIType.RPC, 'goerli');
    ...
    else throw new Error(`EthereumGoerliNetwork: getAPIUrlOfType() has no entry for url type ${type.toString()}`);
  }
}

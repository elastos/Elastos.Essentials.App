...
export class TronWalletJSSafe extends Safe implements TronSafe {
  ...
  public async initialize(networkWallet: AnyNetworkWallet): Promise<void> {
    ...
    if (!this.tronAddress) {
      ...
      if (this.tronAddress) await networkWallet.saveContextInfo('tronAddress', this.tronAddress);
    }
  }
  ...
  private async initTronWeb() {
    ...
    this.tronWeb = new TronWeb({
      fullHost: this.networkWallet.network.getRPCUrl(),
      privateKey: '01'
    });
  }
  ...
  async createTransferTransaction(toAddress: string, amount: string): Promise<any> {
    await this.initTronWeb();
    return this.tronWeb.transactionBuilder.sendTrx(toAddress, amount, this.tronAddress);
  }
  ...
  public async signTransaction(
    ...
    try {
      ...
      if (privateKey) {
        await this.initTronWeb();
        let signedTxn = await this.tronWeb.trx.sign(
          rawTransaction,

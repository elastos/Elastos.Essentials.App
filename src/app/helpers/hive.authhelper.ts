...
import { AppContext, AppContextProvider, VaultServices, VaultSubscriptionService } from "@elastosfoundation/elastos-hive-js-sdk";
...
export class InternalHiveAuthHelper {
  ...
  public async getAppContext(targetDid: string, onAuthError?: (e: Error) => void): Promise<AppContext> {
    ...
    let appContextProvider: AppContextProvider = {
      ...
      getAppInstanceDocument: function (): Promise<DIDDocument> {
   ...
   */
  public getVaultServices(targetDid: string, onAuthError?: (e: Error) => void): Promise<VaultServices> {
      ...
      try {
        ...
        let appContextProvider: AppContextProvider = {
          ...
          getAuthorization: function (authenticationChallengeJWtCode: string): Promise<string> {
            ...
            Logger.log("hiveauthhelper", "Hive client authentication challenge callback is being called with token:", authenticationChallengeJWtCode);
            try {
              return authHelper.handleVaultAuthenticationChallenge(authenticationChallengeJWtCode);
            }
            catch (e) {
              Logger.error("hiveauthhelper", "Exception in authentication handler:", e);
              if (onAuthError)
        ...
        let vaultServices = new VaultServices(appContext, providerAddress);
